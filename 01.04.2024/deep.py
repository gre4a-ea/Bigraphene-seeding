...
class bigraphene:
    ...
    def ShowPlot(self):
        ...
        ax.scatter3D(self.carbon_low[:,0], self.carbon_low[:,1], self.carbon_low[:,2], s=3, alpha = 0.5, color = "black")
    ...
    def StartModelling(self, bounding_border, root_address):
        bounding_indexes_low = ~(self.carbon_low[:,0]**2+ self.carbon_low[:,1]**2 <= (self.radius*bounding_border)**2 ).reshape(-1,1)
        bounding_indexes_high = ~(self.carbon_high[:,0]**2+ self.carbon_high[:,1]**2 <= (self.radius*bounding_border)**2 ).reshape(-1,1)
        

        count = np.count_nonzero(bounding_indexes_low)+np.count_nonzero(bounding_indexes_high)
    

        carbon= np.vstack((self.carbon_high[bounding_indexes_high.flatten()], self.carbon_low[bounding_indexes_low.flatten()], self.carbon_high[~bounding_indexes_high.flatten()], self.carbon_low[~bounding_indexes_low.flatten()]))
...
for j in range(1):
    ...
    x.StartModelling(bounding_border, ROOT_DIR)
    ...
    for i in range(1000):
        ...
        x.ReadXYZ(ROOT_DIR+"/opt.xyz")
        x.ShowPlot()
        x.RemoveH2(0.74)
        x.RemoveHydrogen(1.6)
        x.ShowPlot()
        x.StartModelling(bounding_border, ROOT_DIR)
        x.ShowPlot()
        x.ReadXYZ(ROOT_DIR+"/opt.xyz")
    ...
    #     x.AddHydrogenPartly(excication_part, 0.95)   
    ...
    #     return_code = subprocess.call("mpirun -np 6 --use-hwthread-cpus ~/soft/exe/lmp_mlip < input | tee out", shell=True)
    #     if return_code!=0:
    #         print("Модель сколлапсировала на шаге "+str(2*i+1))
    #         exit()
    #     else: 
    #         print("Модель оптимизирована на шаге "+str(2*i+1))
    #     x.ReadXYZ(ROOT_DIR+"/opt.xyz")
    #     x.ReadEnergy(ROOT_DIR+"/out")
    #     n=np.shape(x.hydrogen_low)[0]+np.shape(x.hydrogen_high)[0]
    #     x.MoveToCSVLogs(subname_m+".csv", n)
