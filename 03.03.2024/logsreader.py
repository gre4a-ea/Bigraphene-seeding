...
for i in range(100):
    if actual_part>0.25:
        actual_part=actual_part*(1-excication_part)
    print(actual_part)
    data3[i]=actual_part    
print(data3)
...
for i in range(4):
    ...
    data1=make_table('logs_m_'+str(i)+'.csv')
    data2=make_table('logs_s_'+str(i)+'.csv')

    ax1.plot( data1[::3,2][-data1[::3,0]<300000]/3, -data1[::3,0][-data1[::3,0]<300000],    color = "red", label="Modernized algorythm")

    ax1.plot( data2[::3,2][-data2[::3,0]<300000]/3, -data2[::3,0][-data2[::3,0]<300000],    color = "blue", label="Straight algorythm")

    ax2 = ax1.twiny()

    new_tick_locations = np.array(data1[::15,2]/3, dtype=int)+5 # Пример пользовательских меток
    new_tick_labels = np.round(1-data3[::5], decimals=2)  # Пример пользовательских подписей
...
ax1.legend()
ax1.set_ylabel(r"$Energy$, -E ", fontsize = 25)
ax1.set_xlabel(r"$Iteration$ ", fontsize = 25)
ax2.set_xlabel("Part of condidates from whole carbons that were chosen to creat interlayer bound ", fontsize = 15)
