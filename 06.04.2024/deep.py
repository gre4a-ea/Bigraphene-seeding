...
class bigraphene:
    ...
    def AddHydrogenPartly(self, part, border):
        ...
        activated_index_high = np.full(np.shape(self.carbon_high)[0], False, dtype=bool).reshape(-1,1)
        activated_index_low = np.full(np.shape(self.carbon_low)[0], False, dtype=bool).reshape(-1,1)
        ...
        if np.shape(self.hydrogen_low)[0] != 0:
            ...
            activated_index_low[nearest_neighbor_index[:,0]] = True
    

            nbrs = NearestNeighbors(n_neighbors=4, algorithm='ball_tree').fit(self.carbon_low)
            distances_low, nearest_neighbor_index = nbrs.kneighbors(self.carbon_low[activated_index_low[:,0]])
            nni=np.hstack((nearest_neighbor_index[:,1],nearest_neighbor_index[:,2],nearest_neighbor_index[:,3]))
            activated_index_low[np.where(np.bincount(nni)>=2)[0]] = True
        ...
        if np.shape(self.hydrogen_high)[0] != 0:
            ...
            distances_high, nearest_neighbor_index = nbrs.kneighbors(self.hydrogen_high)
            activated_index_high[nearest_neighbor_index[:,0]] = True
            
    

            nbrs = NearestNeighbors(n_neighbors=4, algorithm='ball_tree').fit(self.carbon_high)
            distances_high, nearest_neighbor_index = nbrs.kneighbors(self.carbon_high[activated_index_high[:,0]])
            nni=np.hstack((nearest_neighbor_index[:,1],nearest_neighbor_index[:,2],nearest_neighbor_index[:,3]))
            activated_index_high[np.where(np.bincount(nni)>=2)[0]] = True
        ...
        unbound_indexes_low = (self.carbon_low[:,0]**2+ self.carbon_low[:,1]**2 <= (self.radius*border)**2 ).reshape(-1,1)
        unbound_indexes_high= (self.carbon_high[:,0]**2+ self.carbon_high[:,1]**2 <= (self.radius*border)**2 ).reshape(-1,1)

        free_low=np.logical_and( ~activated_index_low, unbound_indexes_low)
        free_high=np.logical_and( ~activated_index_high, unbound_indexes_high)

        distances_high_ = np.sort(distances_high[free_high])
        distances_low_ = np.sort(distances_low[free_low])
        ...
        condidates_low =np.logical_and( (distances_low<=bording_distance), (free_low))
        condidates_high=np.logical_and( (distances_high<=bording_distance), (free_high))

        

        if np.any(condidates_low) and np.any(condidates_high):

            nbrs = NearestNeighbors(n_neighbors=4, algorithm='ball_tree').fit(self.carbon_low)
            distances, nearest_neighbor_index = nbrs.kneighbors(self.carbon_low[condidates_low[:,0]])
            neighbor_low = np.full(np.shape(self.carbon_low)[0], False, dtype=bool).reshape(-1,1)
            neighbor_low[nearest_neighbor_index[:,1]]=True
            neighbor_low[nearest_neighbor_index[:,2]]=True
            neighbor_low[nearest_neighbor_index[:,3]]=True

            nbrs = NearestNeighbors(n_neighbors=4, algorithm='ball_tree').fit(self.carbon_high)
            distances, nearest_neighbor_index = nbrs.kneighbors(self.carbon_high[condidates_high[:,0]])
            neighbor_high = np.full(np.shape(self.carbon_high)[0], False, dtype=bool).reshape(-1,1)
            neighbor_high[nearest_neighbor_index[:,1]]=True
            neighbor_high[nearest_neighbor_index[:,2]]=True
            neighbor_high[nearest_neighbor_index[:,3]]=True
        else:
            ...
            ax.scatter3D(self.carbon_high[activated_index_high[:,0]][:,0], self.carbon_high[activated_index_high[:,0]][:,1], self.carbon_high[activated_index_high[:,0]][:,2], s=3, alpha = 0.5, color = "black")
            ax.scatter3D(self.carbon_low[activated_index_low[:,0]][:,0], self.carbon_low[activated_index_low[:,0]][:,1], self.carbon_low[activated_index_low[:,0]][:,2], s=3, alpha = 0.5, color = "green")
        ...
        free_low=np.logical_and(free_low, ~condidates_low)
        free_high=np.logical_and(free_high, ~condidates_high)

       

        mask_low=np.logical_and(~condidates_low,  neighbor_low)
        mask_high=np.logical_and(~condidates_high, neighbor_high)
