...
colour=["green","blue","red", "purple",  "orange","yellow","grey", "black"]
...
for j in [0,1,2,3,4,5,6]:
    ...
    data1=xyz_to_dataframe('storage/logs_m_'+str(j)+'.xyz')
    ...
    for i in range(iters):
        ...
        hydrogen=result_matrix[result_matrix[:,0]=='AtomType2'][:,1:4].astype(float)
        carbon=result_matrix[result_matrix[:,0]=='AtomType1'][:,1:4].astype(float)
        nbrs = NearestNeighbors(n_neighbors=5, algorithm='ball_tree').fit(carbon)
        distances, nearest_neighbor_index = nbrs.kneighbors(carbon)
        part=np.shape(distances[:,4][distances[:,4]<1.8])[0]
        party=np.shape(hydrogen)[0]
    ...
    plt.plot( x, y,    color = colour[j], label="Modernized algorythm")
    print("plotted")
    ...
    #     nbrs = NearestNeighbors(n_neighbors=5, algorithm='ball_tree').fit(carbon)
    #     distances, nearest_neighbor_index = nbrs.kneighbors(carbon)
    #     part=np.shape(distances[:,4][distances[:,4]<2])[0]
    ...
    # plt.plot( x, y,    color = "blue", label="straight algorythm")
    # print("plotted")
...
plt.legend()
plt.ylabel(r"Number of sp-3 atoms", fontsize = 25)
plt.xlabel(r"Number of hydrogen ", fontsize = 25)
